...
static HAL_StatusTypeDef lsm6dsow_write_reg(lsm6dsow_device_t *device,
                                            uint8_t reg,
                                            uint8_t value)
{
  return HAL_I2C_Mem_Write(device->bus,
                           device->address,
                           reg,
...
static HAL_StatusTypeDef lsm6dsow_read_reg(lsm6dsow_device_t *device,
                                           uint8_t reg,
                                           uint8_t *value,
                                           uint16_t len)
{
  return HAL_I2C_Mem_Read(device->bus,
                          device->address,
                          reg,
                          I2C_MEMADD_SIZE_8BIT,
                          value,
...
HAL_StatusTypeDef lsm6dsow_init(lsm6dsow_device_t *device,
...
HAL_StatusTypeDef lsm6dsow_read_sample(lsm6dsow_device_t *device,
                                       imu_sample_t *sample)
{
  uint8_t gyro_bytes[6];
  uint8_t accel_bytes[6];

  if ((device == NULL) || (sample == NULL) || (device->initialized == 0U)) {
    return HAL_ERROR;
  }
  ...
  sample->gyro_dps.x =
      (float)lsm6dsow_read_i16(&gyro_bytes[0]) * PALM_LSM6DSOW_GYRO_DPS_PER_LSB;
  sample->gyro_dps.y =
      (float)lsm6dsow_read_i16(&gyro_bytes[2]) * PALM_LSM6DSOW_GYRO_DPS_PER_LSB;
  sample->gyro_dps.z =
      (float)lsm6dsow_read_i16(&gyro_bytes[4]) * PALM_LSM6DSOW_GYRO_DPS_PER_LSB;

  sample->accel_g.x =
      (float)lsm6dsow_read_i16(&accel_bytes[0]) * PALM_LSM6DSOW_ACCEL_G_PER_LSB;
  sample->accel_g.y =
      (float)lsm6dsow_read_i16(&accel_bytes[2]) * PALM_LSM6DSOW_ACCEL_G_PER_LSB;
  sample->accel_g.z =
      (float)lsm6dsow_read_i16(&accel_bytes[4]) * PALM_LSM6DSOW_ACCEL_G_PER_LSB;
